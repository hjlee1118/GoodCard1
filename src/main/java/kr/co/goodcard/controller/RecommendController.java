...
public class RecommendController {
	...
	private ArrayList<CreditCard> getCardList(FindIterable<Document> fi) {
		...
		while (cursor.hasNext()) {
			...
			tempCard.setCardType((String) iter.get("cardName"));
...
//				System.out.println(brand.getBrand());
	...
	private ArrayList<Integer> insertPollAndReturnSpendList(Poll poll, Transportation transportation,
		...
		recommendService.insertPoll(poll); recommendService.insertAcademy(academy);
...
//		FindIterable<Document> fi = collection.find(query).limit(500);
