		...
		<div class="container" style="width: 100%;">
			...
			<div style="height: 15px; clear: both;"></div>
				...
				<div id="select_brand_menu"
					...
					<div class="box">
						<c:choose>
							<c:when test="${ creditDataList[28] eq 'i' }">
					...
					<c:set var="creditCardNameNo" value="0"></c:set>

					<c:forEach items="${creditCardNameList}" var="creditCardNameList"
						varStatus="status" begin="0" end="27" step="1">
						<div class="box">
							<c:choose>
								<c:when test="${ creditDataList[creditCardNameNo] eq 'a' }">
									<div class="box-active" title="a" id="menu${creditCardNameNo}"
										onclick="menuActive(this)">${creditCardNameList}</div>
								</c:when>
								<c:otherwise>
									<div class="content" title="i" id="menu${creditCardNameNo}"
										onclick="menuActive(this)">${creditCardNameList}</div>

								</c:otherwise>
							</c:choose>
						</div>
						<c:set var="creditCardNameNo" value="${creditCardNameNo+1 }"></c:set>
					</c:forEach>
				...
				<div id="select_category_menu_button"
					...
					<i class="fa fa-credit" aria-hidden="true"
				...
				<div id="select_category_menu"
					...
					<c:forEach items="${selectCheckCategoryList}" var="selectList"
						...
						<div class="box">
							<c:choose>
								<c:when test="${ creditCategoryList[categoryNo] eq 'a'}">
			...
			<div style="width: 100%;">
				<ul style="padding: 0">
					<li class="custom-sort-design first"><i class="fa fa-credit"
						aria-hidden="true"></i>굿카드 랭킹순</li>
					<li class="custom-sort-design first"><i class="fa fa-credit"
						aria-hidden="true"></i>등록일 순</li>
					<li class="custom-sort-design last"><i class="fa fa-credit"
			...
			<div id="paging-area-5" style="float: center; text-align: center;">
				...
				<c:choose>
					<c:when test="${ pageNo mod 5 ne 0 }">
						...
						<c:choose>
							<c:when test="${ creditTotalPageCnt < pageStartNo + 4 }">
								<c:set var="pageEndNo" value="${ creditTotalPageCnt }" />
							</c:when>
							<c:otherwise>
				...
				<c:if test="${ pageEndNo + 1 < creditTotalPageCnt }">
